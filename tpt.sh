...
# Default values
DEFAULT_TOOLS_DIR="$HOME/tpt-tools"
...
save_config() {
    echo "TOOLS_DIR=\"$TOOLS_DIR\"" > "$CONFIG_FILE"
}
...
# Clone repository with tools
setup_tool_directory() {
    echo -e "${YELLOW}[+] Setting up tool directory...${NC}"
    ...
    if [ ! -d "$TOOLS_DIR/routersploit" ]; then
    ...
    echo -e "${GREEN}[+] Tool directory setup completed${NC}"
}
...
exploitation_tools() {
    ...
    case $choice in
        ...
        2) 
            if [ -d "$TOOLS_DIR/routersploit" ]; then
                cd "$TOOLS_DIR/routersploit"
                python rsf.py
            else
                echo -e "${RED}[!] RouterSploit not installed. Run setup from main menu.${NC}"
            fi
            ;;
        3)
            if [ -d "$TOOLS_DIR/wifite2" ]; then
                cd "$TOOLS_DIR/wifite2"
                python Wifite.py
            else
                echo -e "${RED}[!] Wifite not installed. Run setup from main menu.${NC}"
            fi
            ;;
        *) echo -e "${RED}[!] Invalid option${NC}" ;;
    esac
    ...
    case $choice in
        ...
        3)
            ...
            if [ -d "$TOOLS_DIR/Sublist3r" ]; then
                ...
                echo -e "${RED}[!] Sublist3r not installed. Run setup from main menu.${NC}"
            fi
            ;;
        4)
